...
const char *WINDOWTITLE = { "OpenCL/OpenGL Particle System -- Zhouxiang Meng" };
...
const int NUM_PARTICLES = 1024*1024*30;
const int LOCAL_SIZE    = 32;
const char *CL_FILE_NAME   = { "mengz-particles.cl" };
...
{
        char filename[128];
        sprintf( filename, "mengzparallel.%03d.particles", Frame ); // use your engr login instead
        FILE *fp = fopen( filename, "result" );
        if( fp == NULL )
        {
